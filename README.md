...
| `/addWip`    |             | POST   | Adds a WIP map to the top of the queue<br/>- User identifiers can be tacked on with a `user` query parameter.<br/>- Maps can be appended to the list instead of prepended by tacking on the `prepend` query parameter and setting it to `false`.<br/>- Service identifiers can be added by tacking on the `service` query parameter, useful if your requests come from multiple services.<br/><br/>**Direct URL to a zip file OR a hexadecimal code obtained from wipbot.com uploads is required in POST data.**<br/><br/>`/addWip?user=TheBlackParrot` | [Map Data](#map-data-type)                         |
