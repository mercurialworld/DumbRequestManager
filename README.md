...
| `/addWip`  |             | Adds a WIP map to the top of the queue (via [CatseTheCat's WIP map service](https://catse.net/wip))<br/>- User identifiers can be tacked on with a `user` query parameter.<br/>- Maps can be appended to the list instead of prepended by tacking on the `prepend` query parameter and setting it to `false`.<br/>- Service identifiers can be added by tacking on the `service` query parameter, useful if your requests come from multiple services.<br/>`/addWip/0c93790?user=TheBlackParrot`     | [Map Data](#map-data-type)                         |
